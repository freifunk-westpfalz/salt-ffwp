...
request LE certs for firmware domains:
  cmd.run:
    - name: acmetool quickstart --batch && acmetool --batch want download.freifunk-westpfalz.de updates.freifunk-westpfalz.de download.premium.freifunk-westpfalz.de ffwp.info
...
nginx-firmware-service:
  service.running:
    ...
    - watch:
        ...
        - file: place nginx firmware ffwp info config
...
place nginx firmware ffwp info config:
  file.managed:
    - name: /etc/nginx/sites-enabled/firmware_ffwp_info.conf
    - source: salt://nginx/files/hosts/firmware/firmware_ffwp_info.conf.j2
    - template: jinja